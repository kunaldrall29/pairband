...
Merge templates/cursor-pairband.mdc into the repository's existing .cursor/rules structure after inspecting it. Keep financial requirements in shared docs, not solely editor chat. Reference the specific O-stage file and affected source folders in each task; do not depend on the editor indexing an entire archive.
